package dev.franke.felipee.braspag_automator_v2.checkout_retrieve_merchant_data.service.utils;
...
@Component
public class ProcessExecutionCheckoutData {

  private static final Logger LOG = LoggerFactory.getLogger(ProcessExecutionCheckoutData.class);
  ...
  public byte getNumberOfPythonProcesses() {
    ...
    final String[] processCommand = {
      "/bin/sh", "-c", "ps -ef | grep python | grep -v grep | wc -l"
    };
  ...
  private ProcessBuilder getProcessBuilder(final String ec) {
    ...
    final String fullCommand =
        getScriptForMakingScriptExecutable() + " && " + getExecutableString(ec);
  ...
  private String getScriptForMakingScriptExecutable() {
    return "chmod +x python/prod_checkout_merchant_data.sh";
  }

  private String getExecutableString(final String ec) {
    return "python/prod_checkout_merchant_data.sh " + login + " " + password + " " + ec;
  }
}
